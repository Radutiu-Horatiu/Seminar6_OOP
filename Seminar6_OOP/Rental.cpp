...
void Rental::show_orders() const {
	...
	for (size_t i = 0; i < clients.size(); i++) {
		...
		if (clients[i].getComenzi().size() != 0) {
			cout << "HAS ORDERED: \n";
			for (size_t j = 0; j < clients[i].getComenzi().size(); j++) {
				sleep_for(nanoseconds(300000000));
				cout << clients[i].getComenzi()[j].toString();
			}
...
std::vector<Reservierung> Rental::getOrders() const {
	return orders;
}
std::vector<Reservierung>& Rental::getOrders() {
	return orders;
}
